...
def generate_deformed_projections_LCA():
    ...
    for i in range(num_phantoms):
        ...
        # Distances
        geo.DSD = 1060 + 30*np.random.rand()*random.choice((-1, 1)) # Distance Source Detector      (mm)
        geo.DSO = 750 + 10*np.random.rand()*random.choice((-1, 1)) # Distance Source Origin        (mm)
        geo.offOrigin = np.array([0, 0, 0])  # Offset of image from origin   (mm) #detector view:-z,x,y 

        angle_one_pri = 0 + 8*np.random.rand()*random.choice((-1, 1)) 
        angle_one_sec = 30 + 12*np.random.rand()*random.choice((-1, 1))
        ...
        #############################
        # Distances
        geo.DSD = geo.DSD + 70*np.random.rand()*random.choice((-1, 1)) # Distance Source Detector      (mm)
        ...
        angle_two_pri = -35 + 12*np.random.rand()*random.choice((-1, 1))
        angle_two_sec = 33 + 12*np.random.rand()*random.choice((-1, 1))
...
if __name__ == '__main__':
    ...
    generate_deformed_projections_LCA()
